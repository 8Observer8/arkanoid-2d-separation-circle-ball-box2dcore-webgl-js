...
import { b2CircleShape, b2BodyType, b2PolygonShape } from "@box2d/core";
import { b2World, DrawShapes, b2Vec2 } from "@box2d/core";
...
const blockXPosElement = document.getElementById("blockXPos");
...
blockXPosElement.addEventListener("input", function (evt) {
    const px = parseFloat(blockXPosElement.value);
    const py = blockBody.GetPosition().y;
    blockBody.SetTransformXY(px / pixelsPerMeter, py, 0);
});
...
async function init() {
    ...
    const ballShape = new b2CircleShape(ballRadius / pixelsPerMeter);
    ...
    const px = parseFloat(blockXPosElement.value);
    const blockBodyDef = {
        position: new b2Vec2(px / pixelsPerMeter, 0 / pixelsPerMeter),
        type: b2BodyType.b2_staticBody
    };
